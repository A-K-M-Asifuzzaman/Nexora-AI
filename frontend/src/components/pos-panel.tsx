...
import { CreditCard, Pause, Play, ShoppingCart } from "lucide-react";
...
export function PosPanel() {
  ...
  async function openSession() { setBusy(true); setError(null); try { setSession(await api<Session>("pos/sessions/open", { method: "POST", body: JSON.stringify({ terminal_id: terminalId, opening_float: "0.0000" }) })); } catch (reason) { setError(reason instanceof Error ? reason.message : "Session could not be opened."); } finally { setBusy(false); } }
